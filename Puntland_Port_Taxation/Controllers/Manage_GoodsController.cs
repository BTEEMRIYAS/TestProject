            ...
            if (Session["login_status"] != null)
            {
                ...
                if (z.Contains(2))
                {
                    ...
                    var goods = (from g in db.Goods
                                ...
                                join c in db.Currencies on gtf.currency_id equals c.currency_id
                                where gtf.end_date == end_date
                                orderby g.goods_id ascending
                                select new GoodsModel { goods_id = g.goods_id, goods_name = g.goods_name, goods_type = gp.goods_type_name, goods_sub_category = gs.goods_subcategory_name, goods_category = gc.goods_category_name, goods_tariff = gtf.goods_tariff, ispercentage = gtf.ispercentage, unit_of_measure = u.unit_code, currency = c.currency_code, tariff_document = gtf.dcument_name, goods_code = g.goods_code }).Skip(start_from).Take(9);
